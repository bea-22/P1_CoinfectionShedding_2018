...
## Part 3 XXXXXX : Bd: model comparison

Testing what is caused the shift in *Alytes muletensis* model predicitions: including the babies (ExpNo. 5) or changing the error structure 

```{r Bd Am load df, results='hide', warning=FALSE, error=FALSE, message=FALSE}
Bd.load.Am <- data.endpoint %>%
  filter(!ExperimentNo=='5')  %>%   # removal of Alytes babies
  filter((Bd.endpoint.status=='1' & Bd.endpoint.GE > 0.1))  %>% 
  dplyr::select(ID, Species, ExperimentNo, Scenario, Treatment, Bd.endpoint.status, Bd.endpoint.GE) 

droplevels(Bd.load.Am)


Bd.load.Ab <- data.endpoint %>%
  filter((Bd.endpoint.status=='1' & Bd.endpoint.GE > 0.1))  %>% 
  dplyr::select(ID, Species, ExperimentNo, Scenario, Treatment, Bd.endpoint.status, Bd.endpoint.GE) 

droplevels(Bd.load.Ab)
```


```{r Bd Am load glm}

Bd.Ab.guass <- glm(log(Bd.endpoint.GE) ~ ExperimentNo, data=Bd.load.Ab, family = gaussian())

Bd.Ab.gamma <- glm(Bd.endpoint.GE ~ ExperimentNo, data=Bd.load.Ab, family="Gamma"(link='log'))


Bd.Am.guass <- glm(log(Bd.endpoint.GE) ~ ExperimentNo, data=Bd.load.Am, family = gaussian())

Bd.Am.gamma <- glm(Bd.endpoint.GE ~ ExperimentNo, data=Bd.load.Am, family="Gamma"(link='log'))

```

```{r Bd Am fitted values, echo=FALSE, fig.cap= "Fig. 6. Plot of relationship between the fitted values from the model and the true data values."}

Bd.load$Am.gaus.fit <- Bd.Am.guass$fitted.values  # add logisitic fitted values back to the dataframe as a new col

Bd.load$Am.gamma.fit <- Bd.Am.gamma$fitted.values  # add logisitic fitted values back to the dataframe as a new col

Bd.load$log.GE <- log(Bd.load$Bd.endpoint.GE)  # add logisitic fitted values back to the dataframe as a new col

names(Bd.load)

Bd.fits <- Bd.load %>%
dplyr::select(Species, Treatment, Bd.endpoint.GE, log.GE, Am.gaus.fit, Am.gamma.fit)

write.table(Bd.fits, file = "Bd_model_fit.txt", sep = ",")

```



## Part 3a: Bd: model selection 
...
**Conclusion** Gamma with log-link as "the most appropriate link function is one which produces minimum residual deviance" and the diagnostic plots are better looking than the gaussian model. 

**To Do** reading on Gamma error structure and log-link function << what do they mean for interpretation?? 

## Part 3b: Bd: model comparison

The four models (all using Gamma with log-link structure)
```{r Bd load glm}

Bd.load1 <- glm(Bd.endpoint.GE ~ Treatment * ExperimentNo, data=Bd.load, family="Gamma"(link='log'))

Bd.load2 <- glm(Bd.endpoint.GE ~ Treatment + ExperimentNo, data=Bd.load, family="Gamma"(link='log'))

Bd.load3 <- glm(Bd.endpoint.GE ~ Treatment, data=Bd.load, family="Gamma"(link='log'))

Bd.load4 <- glm(Bd.endpoint.GE ~ ExperimentNo, data=Bd.load, family="Gamma"(link='log'))

Bd.load.N <- glm(Bd.endpoint.GE ~ 1, data=Bd.load, family="Gamma"(link='log'))

```
<br>

```{r Bd load model comparison}
anova(Bd.load1, Bd.load2, test="Chisq") # start by comparing the interaction terms  

anova(Bd.load2, Bd.load3, test="Chisq") # compares Trt and Species to just Trt 

anova(Bd.load2, Bd.load4, test="Chisq")  # compares Trt and Species to just Species  

anova(Bd.load4, Bd.load.N, test="Chisq")  # compares Species model to null model 

## QUESTION should I change the `test = ` to better reflect the data type?  AF response "yes, unless you find a statement against it. F stats are used for quasi models"

```

ANOVA 1: suggests we should reject the more complex model with interaction terms (Treatment * ExperimentNo) in favour for the model with just the terms  (pvalue = 0.4362)
<br>
...
ANOVA 3: suggests we should reject the more complex model (Treatment + ExperimentNo) and choose the model with just ExperimentNo. (pvalue = 0.08907) 
<br>
...
Tukey.Bd.load <- glht(Bd.load4, linfct=mcp(ExperimentNo='Tukey'))
...
Also did a ```pairwise.t.test()``` comparing the factor levels of ExperimentNo .... not entirely sure wht 

```{r Bd - posthoc - adjusted pvalues , echo=FALSE, warning=FALSE, error=FALSE, message=FALSE, fig.width=8, fig.height=5}
attach(Bd.load)
pairwise.t.test(Bd.endpoint.GE, ExperimentNo)
```
...
- QQ plot is not great, suggesting not normally distributed
...
**Conclusion** Gamma with log-link has the best (QQ) plots and lowest residual deviance 
...
## Part 6b: Rv: model comparison

The four models (all using log(Bd load) and gaussian error structure)
```{r Rv load glm}
Rv.load1 <- glm(Rv.endpoint.load ~ Treatment * ExperimentNo, data=Rv.load, family="Gamma"(link='log'))

Rv.load2 <- glm(Rv.endpoint.load ~ Treatment + ExperimentNo, data=Rv.load, family="Gamma"(link='log'))

Rv.load3 <- glm(Rv.endpoint.load ~ Treatment, data=Rv.load, family="Gamma"(link='log'))

Rv.load4 <- glm(Rv.endpoint.load ~ ExperimentNo, data=Rv.load, family="Gamma"(link='log'))

Rv.load.N <- glm(Rv.endpoint.load ~ 1, data=Rv.load, family="Gamma"(link='log'))
```

```{r Rv load model comparison}
anova(Rv.load1, Rv.load2, test="Chisq") # start by comparing the interaction terms  

anova(Rv.load2, Rv.load3, test="Chisq") # compares Trt and Species to just Trt 

anova(Rv.load2, Rv.load4, test="Chisq")  # compares Trt and Species to just Species  

anova(Rv.load3, Rv.load.N, test="Chisq")  # compares Species model to null model 

```


ANOVA 1: shows adding a interaction term between Treatment and ExperimentNo does not greatly imporve fit (p-value = 0.08616)
<br>

ANOVA 2: suggests we should reject the more complex model (Treatment + ExperimentNo) in favour of the model with Treatment only  (pvalue = 0.2225)  
<br>

ANOVA 3: suggests we should favour the more complex model (Treatment + ExperimentNo) over the model with ExperimentNo only  (pvalue = < .001)   
<br>

ANOVA 4: suggests we should accept the more complex model (Treatment) over the null model  (pvalue = < .001)  so ExperimentNo leads to significantly improved fit 
<br>

**Conclusion**: we should choose the model with just Treatment (like Rv status)


## Part 6c: Rv: model fit
...
## Part 6e: Rv: model checks 

Here I check the two best models.

#### Maximal Model: Species + Treamtent (no interaction)

```{r Rv - glm - checks1, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE}
# Look at the estimates of the coefficients using ```summary()```

#Rv.load2 <- glm(Rv.endpoint.load ~ Treatment + ExperimentNo, data=Rv.load, family="Gamma"(link='log'))
summary(Rv.load2)   # Residual deviance: 22.027  on 14 df   AIC: 255.8
```

```{r Rv - glm - checks1 plot, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE }
par(mfrow=c(2,2), mar=c(3,3,3,1), mgp=c(2,0.8,0))
plot(Rv.load2)
```
...
```{r Rv - glm - checks2, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE}
#Rv.load3 <- glm(Rv.endpoint.load ~ Treatment, data=Rv.load, family="Gamma"(link='log'))
summary(Rv.load3)   # Residual deviance: 24.287  on 16 df   AIC: 253.89

Rv.load.coef <-summary.glm(Rv.load3)$coefficients
```

```{r Rv - glm - checks2 plot, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE}
par(mfrow=c(2,2), mar=c(3,3,3,1), mgp=c(2,0.8,0))
plot(Rv.load3)
```

The AIC better for Treatment only model. 
...
Tukey.Rv.load <- glht(Rv.load3, linfct=mcp(Treatment='Tukey'))
