...
We exposed all larval individually to controlled doses of Bd, Rv or the correlating sham media (see Fig.1.). 

####
The differential number of exposures for the pathogens was chosen based on the nature of the pathogens. Rv was considered more pathogenic (able to infect in one dose), were as Bd infection is normally acquired by multiple exposures with infection building over time. For individuals in the co-infection groups, doses of the parasites were given sequentially reflecting knowledge of the pathogens present in the wild and their role as emerging pathogens. Previous studies have also shown that repeatedly exposed to Bd increases the likelihood of individuals developing infections as aposed to one single, high dose [REFERENCE: Daversa, et al., (2018) Functional ecology]
####

Tadpoles were exposed to their respective dose by directly pipetting the inoculums into 0.075L Really Useful Boxes (here on referred to as housing container(s)) filled with 210mL aged tap water, after a 100% water change. Following the 6-hour exposure period, we raised the water volume to 375mL to maintain water quality. The *A.muletensis* I tadpoles being larger were housed in 1.7L RUBs with 1L aged tap water which we reduced to 520mL for exposure periods.  
...
We conducted water parameter tests throughout the experiment on both the housing and shedding containers to test for nitrates, nitrites and ammonia levels. All larvae were fed 200µL of ground Tetra Tabimin tablets and Spirulina dispersed in double-distilled water to the ratio 1g/100mL, bar *A.muletensis* I who recieved double fed, every other day. The temperature of the animal rooms followed natural ambient conditions (averaging 20˚C ± 2) with a 14:10 light:dark schedule.
...
compare host response between species at a single time point postinoculation;

Infection status and load was determined for each individual from endpoint tissue samples. On Day 23 **(9 days post infection (dpi))** tadpoles were euthanised, under licence, by buffered (pH 7.0) 5 mg/L tricaine methylsulfonate (MS-222) and then stored in 100% ethanol for subsequent molecular quantification of infection burden. Infection by Bd was confirmed by excision and DNA extraction of mouthparts, the site of Bd colonization and infection in tadpoles [REFERENCE], using Prepman Ultra (Life Technologies) as per Hyatt et al. (2007). Extracts were screened by the qPCR diagnostic, targetting the ITS-1 and 5.8S regions, outlined in Boyle et al. (2004) with modifications from Garner et al. (2009). We express Bd infection load in genomic equivalents (GE) where one GE is equivalent to a single zoospore. 
...
We used generalized linear models (GLMs) to test the effect of exposure scenario (Bd only, Rv only, Bd-Rv and Rv-Bd) and host species (*B.bufo*, *R.temporaria* and *A.muletensis*) on infection status (uninfected vs. infected), using a binomial error structure, and on infection load using a Gamma error structure with log-link transformation. **Model parameters were assessed using analysis of deviance (ANOVA). << include this here**. Where the response variable was infection load it was recorded as GE score for Bd positive infections and as a normalised viral load for ranavirus. **<< comment here about why se didnt log the Bd GE score??? ** Each of level of statistical comparison (exposure scenario, species) on the response variable (infection status or load) was carried out individual for each pathogen. 

We implemented/used Tukey’s ((honest significant difference)) post hoc test to compare // to evaluate the significant effects of the explanatory vairables highlighted by the GLMs, using the glht function in the multcomp package; [ref], 
...
We have excluded *A.muletensis* II from the ((above mentioned))) models looking at the ########## infection status and load, as that experiment did not have the full range of exposure scenarios. To test the difference in development stage on Bd infection status and load we compare *A.muletensis* I and less developed *A.muletensis* II using #######. 
...
Survivorship was analysed using Cox proportional hazard models, again with exposure scenario and host species as explanatory vairables, with the ‘‘coxph’’ function and the Survival package.
...
boxplot.Rv <- plot.Rv.load.sp +  theme(legend.key=element_blank(), legend.position="top",legend.justification='right', legend.direction='horizontal', legend.text= element_text(size=8, face = "italic"), legend.title=element_text(size=14)) + 
     theme(axis.text.x = element_text(angle=30, hjust=1, vjust=1, size = 10), axis.text.y = element_text(size = 10), axis.title = element_text(size=16)) 
...
Exposure scenario predicted infection status and load for ranavirus: (co-infected) tadpoles exposed to both pathogens were **significantly** more likely to develop ranaviral infections (**binomial** GLM; Bd-Rv Z = -5.667; p < 0.001, ((Rv-Bd Z = 2.924; p < 0.01))); Figure XXXXX), which were more likely to be at a high viral titre (GLM; χ2 = XXX; p < .001 ****, Figure XXXXX ////. Across species the endpoint viral load of individuals differed between the two co-infection scenarios (Tukey's XXXXXXX)   

Host species did not affect ranavirus infection status or load, nor did we detect an interaction between host species and treatment group/eposure scenario.


Rv only = XXXX ± XXXX, Bd - Rv = XXX ± XXX, Rv - Bd = XXXX ± XXX, GLM, F = XXXX, p < .001; XXXX). 

###########################################
strength predicted infection risk: newts in the high dose tanks were more likely to develop infections (GLM; χ2
1 = 18.44; p < .001, Figure 2a)
andand developed stronger infections (low dose GE = 0.44 ± 0.15, high dose = 8.82 ± 2.72, GLM, F1,51 = 24.67, p < .001; Figure 2b). Risk did not affect how quickly newts first switched to terrestrial
habitat (no risk tdepart = 0.54 ± 0.08, low risk tdepart = 0.55 ± 0.09, high risk tdepart = 0.70 ± 0.08, GLM, F1,51 = 1.66, p = .196). Neither risk, in- fection status, nor infection load significantly affected cumulative tter- restrial (Table S2) or when breaking analysis down by Bd growth phases (Table
 
###########################################
(GLM with binomial distribution, behaviour Z = 7!65, P < 0!00001; immunity Z = 5!32, P < 0!00001; behaviour*immunity Z = #5!09, P < 0!00001). Depression
...
with Treatment xxx for predicting ranavirus infection status and ExperimentNo. best for Bd infection status. 
