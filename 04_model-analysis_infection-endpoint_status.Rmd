...
anova(Bd.status3, Bd.status4, test="Chisq")  # compare Trt to Species model 
...
ANOVA 4: suggests we should favour the model with ExperimentNo. over one with Treatment (pvalue = < .001) 
<br>

ANOVA 5: suggests we should favour the more complex model (ExperimentNo.) over the null model  (pvalue = < .001)  so ExperimentNo leads to significantly improved fit 
...
> *Note* To see the fitted values from a regression object (the values of the dependent variable predicted by the model), access the ```fitted.values``` attribute from a regression object with ````$fitted.values```.
...
```{r Bd - bi.glm - residuals2 plot, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE, fig.width=8, fig.height=5}
